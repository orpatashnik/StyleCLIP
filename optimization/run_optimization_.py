...
def main(args):
    ...
    for i in pbar:
        ...
        if (
            ...
            torchvision.utils.save_image(
                result_gen05["image"],
                os.path.join(args.results_dir, f"{str(i).zfill(5)}05.jpg"),
                normalize=True,
                range=(-1, 1),
            )
            with torch.no_grad():
            ...
            torchvision.utils.save_image(
                result_gen2["image"],
                os.path.join(args.results_dir, f"{str(i).zfill(5)}2.jpg"),
                normalize=True,
                range=(-1, 1),
            )
            with torch.no_grad():
            ...
            torchvision.utils.save_image(
                result_gen3["image"],
                os.path.join(args.results_dir, f"{str(i).zfill(5)}3.jpg"),
                normalize=True,
                range=(-1, 1),
            )
